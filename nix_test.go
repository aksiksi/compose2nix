...
func TestSystemdUnitDescriptions(t *testing.T) {
	for _, runtime := range []ContainerRuntime{ContainerRuntimeDocker, ContainerRuntimePodman} {
		t.Run(runtime.String(), func(t *testing.T) {
			config := &NixContainerConfig{
				Runtime:  runtime,
				Networks: []*NixNetwork{{Runtime: runtime, Name: "test-network"}},
				Volumes:  []*NixVolume{{Runtime: runtime, Name: "test-volume"}},
				Builds: []*NixBuild{{
					Runtime:       runtime,
			...
			for _, description := range []string{
				`unitConfig.Description = "Network test-network generated by compose2nix.";`,
				`unitConfig.Description = "Volume test-volume generated by compose2nix.";`,
				`unitConfig.Description = "Build for test-container generated by compose2nix.";`,
			} {
				if !strings.Contains(output, description) {
					t.Errorf("missing %q", description)
				}
			}
		})
	}
