...

{
  ...
  systemd.user.services."podman-myproject-entrypoint" = {
    serviceConfig = {
  ...
  systemd.user.services."podman-myproject-no-restart" = {
    serviceConfig = {
      Restart = lib.mkOverride 90 "no";
    };
    after = [
  ...
  systemd.user.services."podman-myproject-service-a" = {
    serviceConfig = {
      Restart = lib.mkOverride 90 "no";
  ...
  systemd.user.services."podman-service-b" = {
    serviceConfig = {
  ...
  # Networks
  systemd.user.services."podman-network-myproject_another" = {
    path = [ pkgs.podman ];
    serviceConfig = {
      Type = "oneshot";
